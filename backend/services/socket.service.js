...
// In-memory rehearsal sessions
const rehearsalSessions = {}
...
export function setupSocketAPI(http) {
    ...
    gIo.on('connection', socket => {
        ...
        socket.on('rehearsal-join', ({ sessionId, userId }) => {
            const session = rehearsalSessions[sessionId]
            if (!session || !session.isActive) {
                socket.emit('rehearsal-not-found')
                return
            }
        ...
        socket.on('rehearsal-end', ({ sessionId }) => {
            const session = rehearsalSessions[sessionId]
            if (!session) return
